...
const Slide = ({ id, image, post, date, desc, valueIndex, index, design, link }) => {
    ...
    <>
      <article className={`d_flex ${position}`} key={id}>
        ...
        <div className='left box_shodow'>
          <div className="tit">{design}</div>
          <div className='img'>
            <img src={image} alt='' />
          </div>
          <button className=' view-btn' onClick= "window.open{link}">See more
          </button>
        </div>
